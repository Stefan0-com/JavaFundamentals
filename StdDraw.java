...
public final class StdDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
    ...
    // modified 12/22/2012
    public static boolean lastFrameVisible = false;
    public static Bag<JFrame> frameBag = new Bag<JFrame>();
    ...
    private static void init() {
        ...
        if ((frame != null) && (! lastFrameVisible)) frame.setVisible(false);
        frameBag.add(frame);
        frame = new JFrame();
    ...
    *************************************************************************/
    ...
    // Modified 12/23/2012
    public static void line(double x0, double y0, double x1, double y1, double offset) {
        double dx = x1 - x0;
        double dy = y1 - y0;
        double dz = Math.sqrt(dx * dx + dy * dy);
        double sine = dy / dz;
        double cosine = dx / dz;
        double dx_offset = - offset * sine;
        double dy_offset = offset * cosine;
        offscreen.draw(new Line2D.Double(scaleX(x0 + dx_offset), scaleY(y0 + dy_offset), 
                scaleX(x1 + dx_offset), scaleY(y1 + dy_offset)));
        draw();
    }
